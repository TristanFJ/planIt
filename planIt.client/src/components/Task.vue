<template>
  <div class="row task my-3 text-center justify-content-evenly border border-warning pt-2 align-self-center">
    <div class="col-md-1 p-0">
      <i v-if="account.id == task.creatorId" class="selectable mdi mdi-pencil" data-bs-toggle="modal"
        :data-bs-target="'#edit-' + task.id" title="Edit"></i>
    </div>
    <div class="col-md-1 p-0">
      <i class="
          selectable
          ...
          rounded
          align-self-center
        " data-bs-toggle="modal" :data-bs-target="'#note-' + task.id" title="Note"></i>
    </div>
    <div class="col-md-1 p-0">
      <input type="checkbox" name="completed" id="completed" @click="completedTask(task.id)" v-model="task.completed" />
    </div>
    ...
    <div class="col-md-1 p-0">
      <i v-if="account.id == task.creatorId" @click="remove()"
        class="selectable mdi mdi-trash-can-outline text-danger"></i>
    </div>
  </div>
        ...
        <div class="modal-body">
          <form @submit.prevent="edit()">
            ...
            <button v-if="account.id == task.creatorId" type="submit" @click.prevent="edit()" class="btn btn-primary"
              :data-bs-target="'#edit-' + task.id" data-bs-dismiss="modal">
              Submit
            </button>
          </form>
        </div>
