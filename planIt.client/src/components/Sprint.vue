    ...
    <div class="mt-5">
      <div class="container card m-2 bg-cDark elevation-3">
        ...
        <form @submit.prevent="createTask()">
          <div class="row">
            <div class="col-8">
              <input v-model="editable.name" type="text" class="form-control bg-light" placeholder="Task Name"
                aria-label="Example text with button addon" aria-describedby="button-addon1" required />
            </div>
            <div class="col-4">
              <input v-model="editable.weight" type="number" min="1" max="5" pattern=".{1, 5}"
                class="form-control bg-light" placeholder="Weight" aria-label="Example text with button addon"
                title="How important is this task?" aria-describedby="button-addon1" required />
            </div>
          </div>
          <button type="submit" @click.prevent="createTask()" class="btn btn-primary my-3"
            data-bs-target="#createSprint" data-bs-dismiss="modal">
            Create
          </button>
        </form>
      </div>
